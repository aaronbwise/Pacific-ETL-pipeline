...
def fiji_r2_analyze_data(df, svy_id):
    """Function to analyze Fiji R2 data"""
    ...
    df.loc[(df['ADM1INName'] == 'Naitasiri') | (df['ADM1INName'] == 'Namosi') | (df['ADM1INName'] == 'Rewa') |\
        (df['ADM1INName'] == 'Serua') | (df['ADM1INName'] == 'Tailevu') | (df['ADM1INName'] == 'Kadavu') |\
                    (df['ADM1INName'] == 'Lau') | (df['ADM1INName'] == 'Lomaiviti') | (df['ADM1INName'] == 'Rotuma'),\
    ...
    df['CARI_inc_regular'] = np.where((df['HHIncFirst'] == 'Salary_Wage_work') | (df['HHIncFirst'] == 'Farming') |\
        (df['HHIncFirst'] == 'Own_business_trade') | (df['HHIncFirst'] == 'Fishing_aquaculture') |\
                (df['HHIncFirst'] == 'Petty_trade_small_business'), 'Yes', 'No')
    ...
    df['LhCSIDomAsset_Y'] = np.where((df['LhCSIDomAsset_YN'] == 'Yes') | (df['LhCSIDomAsset'] == 'Already_depleted'), 1, 0)         # Stress
    df['LhCSIHealth_Y'] = np.where((df['LhCSIHealth_YN'] == 'Yes') | (df['LhCSIHealth'] == 'Already_depleted'), 1, 0)               # Crisis
    df['LhCSIProdAsset_Y'] = np.where((df['LhCSIProdAsset_YN'] == 'Yes') | (df['LhCSIProdAsset'] == 'Already_depleted'), 1, 0)      # Crisis
    df['LhCSISaving_Y'] = np.where((df['LhCSISaving_YN'] == 'Yes') | (df['LhCSISaving'] == 'Already_depleted'), 1, 0)               # Stress
    df['LhCSIBorrowCash_Y'] = np.where((df['LhCSIBorrowCash_YN'] == 'Yes') | (df['LhCSIBorrowCash'] == 'Already_depleted'), 1, 0)   # Stress
    df['LhCSIResAsset_Y'] = np.where((df['LhCSIResAsset_YN'] == 'Yes') | (df['LhCSIResAsset'] == 'Already_depleted'), 1, 0)         # Emergency
    df['LhCSIOutSchool_Y'] = np.where((df['LhCSIOutSchool_YN'] == 'Yes') | (df['LhCSIOutSchool'] == 'Already_depleted'), 1, 0)      # Crisis
    df['LhCSIFemAnimal_Y'] = np.where((df['LhCSIFemAnimal_YN'] == 'Yes') | (df['LhCSIFemAnimal'] == 'Already_depleted'), 1, 0)      # Emergency
    df['LhCSIBegged_Y'] = np.where((df['LhCSIBegged_YN'] == 'Yes') | (df['LhCSIBegged'] == 'Already_depleted'), 1, 0)               # Emergency
    df['LhCSIAnimals_Y'] = np.where((df['LhCSIAnimals_YN'] == 'Yes') | (df['LhCSIAnimals'] == 'Already_depleted'), 1, 0)            # Stress

    # Set levels
    df['LhCSI_Stress'] = np.where((df['LhCSISaving_Y'] == 1) | (df['LhCSIBorrowCash_Y'] == 1) | (df['LhCSIDomAsset_Y'] == 1) | (df['LhCSIAnimals_Y'] == 1), 2, 0)
