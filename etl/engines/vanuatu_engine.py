...
class VanuatuEngine:
    ...
    def run_vanuatu_engine(self):
        vanuatu_tableau = self.generate_output_long()
        return vanuatu_tableau
    ...
    def generate_output_long(self):
        ...
        # Save combined df
        fn = 'vanuatu_analysed_combined.csv'
        path = self.datadir.joinpath(fn)

        combined_df.to_csv(path, index=False)
        print('vanuatu_analysed_combined.csv SAVED!')
    ...
    def clean_combine(self):
        # Get list of vanuatu analysed datasets
    ...
    def add_weights(self, df):
        ...
        # Merge in pop data
        path = self.popdir.joinpath('vanuatu_pop.csv')  # --> -------------  Production  -------------
    ...
    def clean_round(self, df):
        ...
        # Get survey dates
        vanuatu_dates = self.config_data['dates_dict']['Vanuatu']  # --> Refactor?? Generalise?

        # Generate list of round labels
        round_label = ['R' + str(i+1) for i in range(len(vanuatu_dates))]

        # Convert dates to datetime
        start_dates = [pd.to_datetime(vanuatu_dates[label]['start']).tz_localize(tz='UTC') for label in round_label]
        end_dates = [pd.to_datetime(vanuatu_dates[label]['end']).tz_localize(tz='UTC') for label in round_label]
    ...
    def drop_cols(self, df):
        keep_cols_path = self.vardir.joinpath('vanuatu_variables.csv')
