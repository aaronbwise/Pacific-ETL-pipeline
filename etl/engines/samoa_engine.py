...
class SamoaEngine:
    ...
    def run_samoa_engine(self):
        samoa_tableau = self.generate_output_long()
        return samoa_tableau
    ...
    def generate_output_long(self):
        ...
        # Save combined df
        fn = 'samoa_analysed_combined.csv'
        path = self.datadir.joinpath(fn)

        combined_df.to_csv(path, index=False)
        print('samoa_analysed_combined.csv SAVED!')
    ...
    def clean_combine(self):
        # Get list of Samoa analysed datasets
    ...
    def add_weights(self, df):
        ...
        # Merge in pop data
        path = self.popdir.joinpath('samoa_pop.csv')  # --> -------------  Production  -------------
    ...
    def clean_round(self, df):
        ...
        # Get survey dates
        samoa_dates = self.config_data['dates_dict']['Samoa']  # --> Refactor?? Generalise?

        # Generate list of round labels
        round_label = ['R' + str(i+1) for i in range(len(samoa_dates))]

        # Convert dates to datetime
        start_dates = [pd.to_datetime(samoa_dates[label]['start']).tz_localize(tz='UTC') for label in round_label]
        end_dates = [pd.to_datetime(samoa_dates[label]['end']).tz_localize(tz='UTC') for label in round_label]
    ...
    def drop_cols(self, df):
        keep_cols_path = self.vardir.joinpath('samoa_variables.csv')   # --> -------------  Production  -------------
